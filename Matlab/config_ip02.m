...
function [ Rm, Jm, Kt, eta_m, Km, Kg, eta_g, M, r_mp, Beq ] = config_ip02( IP02_LOAD_TYPE )
    % Calculate useful conversion factors
    calc_conversion_constants();
    % Calculate IP01 or IP02 model parameters
    [ Rm, Jm, Kt, eta_m, Km, Kg, eta_g, M, r_mp, Beq ] = calc_IP02_parameters( IP02_LOAD_TYPE );
end


%% Calculate the IP01 or IP02 model parameters 
function [ Rm, Jm, Kt, eta_m, Km, Kg, eta_g, M_c, r_mp, B_c ] = calc_IP02_parameters( IP02_LOAD_TYPE )
    global K_IN2M K_D2R K_RDPS2RPM K_OZ2N % Conversion constants
    ...
    % Set the following calibration constants/parameters: K_EC, K_EP, K_PC, K_PP, M, Beq
        ...
        global K_EC K_EP % Encoder conversion constants
