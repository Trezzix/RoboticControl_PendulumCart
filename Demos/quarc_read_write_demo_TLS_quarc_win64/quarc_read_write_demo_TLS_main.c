...
typedef RT_MODEL_quarc_read_write_dem_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * quarc_read_write_demo_TLS(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(quarc_read_write_demo_TLS_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://quarc_read_write_demo_TLS:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: quarc_read_write_demo_TLS -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
